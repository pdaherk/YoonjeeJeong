...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		list.add("빅뱅");//인덱스 1
		list.add("트와이스");//인덱스 2
		list.add(3, "워너원");//인덱스 3
		//list.add(5, "소방차 ㅋㅋ");//인덱스 4를 건너뛰어서 에러남
		list.add(list.size(), "오마이걸");//위를 주석처리해서 4번 인덱스
		list.add(list.size(), "방탄");//5번 인덱스
		System.out.println("중복 저장전 객체수:"+ list.size());
		
		/* 1-2.중복저장
		 	List는 배열의 속성을 가지므로 중복저장이 허용됨.
		 	add()메소드는 입력에 성공한 경우 true를 반환한다.*/
		System.out.println(list.add("트와이스")? "중복저장됨" : "중복저장 안 됨");//true:false이다
		...
		/* 1-6. 포함여부 확인:
		 		contains(객체명): 찾고자 하는 객체가 있는 경우 true반환
		 		이건 존재 유무만 알려주니 검색용은 아니다*/
		...
		System.out.println(list.remove("방탄")? "삭제됨" : "삭제실패");
		list.remove("오마이걸");
