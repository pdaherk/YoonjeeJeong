...
=> 기존 Object 기반의 FruitBox를 아래와 같이 제네릭 클래스로 정의한다.
...
public class Ex02GenericClass {
	...
	public static void main(String[] args) {
		/*인스턴스 생성시 T부분을 경정하므로 구현의 편의성이 보장된다. 
		 각각 Appple, Banana 객체를 저장할 수 있는 인스턴스가
		 생성되었다. */
		...
		/*OrangeBox는 인스턴스 생성시 Orange를 저장할 수 있는
		 용도로 생성되었기 때문에 다른 객체는 저장 불가다. 
		 아래 코드에서 컴파일 에러가 발생되므로 자료형에도 안전한 코드가 됨.*/
		GenericFruitBox<Orange> orangeBox = new GenericFruitBox<Orange>();
		orangeBox.store(new Orange(15));
		Orange orange = orangeBox.pullOut();
		orange.showInfo();
//		orangeBox.store("난 오렌지");<-컴파일 에러
