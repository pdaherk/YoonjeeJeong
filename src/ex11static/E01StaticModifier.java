...
/* 정적변수(static variable): 
 -멤버변수 앞에 static 지정자가 붙으면 정적변수가 됨.
 -정적변수는 JVM에 의해 프로그램이 로드되는 시점에 초기화됨.
 -main 메소드가 실행되기 전에 초기화 되므로 main메소드실행시
...
class MyStatic {
	...
	/*
	 ...
	 접근이 가능하다. 프로그램 시작시 이미 메소드 영역에 초기화된다. 
	 */
	static int staticVar;
	/*
	 [인스턴스형 멤버메소드] 인스턴스형 메소등에서는 모든 멤버를
	 사용할 수 있다. 인스턴스형변수, 정적변수, 둘다 가능하다
	 */
	void instanceMethod() {
	...
	/*
	 [정적멤버메소드] 정적메소드 안에선 인스턴스 멤버를 사용할 수 없다.
	 정적멤보만 사용가능함.
	 (main 메소드에서는 static으로 선언된 메소드만 호출할 수 있다!!!) 
	 */
	static void staticMethod() {
//		System.out.println("instanceVar="+instanceVar);//오류
		System.out.println("staticVar="+staticVar);//오류
//		instanceMethod();//오류
	}
}
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/* static(정적) 멤버변수 접근방법
		 방법1 : 클래스명을 통해서 접근한다. 객체 생성없이 바로
		 접근하는 것이 가능하다 */
		MyStatic.staticVar = 200;
		System.out.println("myStatic.instanceVar="+
				myStatic.staticVar);
		/* 방법2: 인스턴스변수처럼 참조변수를 통해 접근할 수 있다.
		 단, 이 경우는 정적변수를 인스턴스변수로 착각할 수 있으므로
		 권장하지 않는다 */
		...
		/*
		 인스턴스 멤버면수는 객체별로 각각 존재하므로 아래 초기화된 값은 객체별로 각각
		 저장된다. 따라서 출력결과는 100, 1000이 된다
		 */
		ms1.instanceVar = 100;
		...
		/*
		 정적멤버는 인스턴스화된 객체마다 따로 있는 멤버가 아니라 메소드영역에 
		 하나만 생성되어 모든 객체가 공유하는 공유변수가 된다. 따라서 아래 출력결과는
		 900, 900이 된다. 
		 */
		ms1.staticVar = 800;
		ms2.staticVar = 900;/*
			위 두개의 문장은 동일한 변수에 접근하고 있으나 코드 레벨에서는
			마치 다른 변수처럼 보인다. 따라서 위 문장은 MyStatic.staticVar로
			기술하는 것이 좋다.*/
		
		System.out.println("ms1.staticVar="+ ms1.staticVar);//900
		System.out.println("ms2.staticVar="+ ms2.staticVar);//900

		//정적 멤버메소드의 접근 방법도 변수와 동일한 규칙을 가진다.
		myStatic.instanceMethod();
		myStatic.staticMethod();
		
		//MyStatic.instanceMethod();//에러. 참조변수를 통해서만 호출가능하다.		
		MyStatic.staticMethod();
