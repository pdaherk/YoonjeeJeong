...
/*	상속(inheritance):
 		...
 		클래스를 정의하는 것을 말한다
 		부모클래스를 자식클래스가 상속할때는 extends키워드를 사용! */
//부모클래스(혹은 상위, 기본, super): 사람의 일반적인 형태를 추상화
class Man{
	private String name;
	/*
...
class BusinessMan extends Man{//extends로 부모클래스 Man을 상속받음
	...
	public BusinessMan(String name, String company, 
			String position) {
		super(name);//부모클래서에서 상속받은 부분임
		/*super는 부모클래스의 매재변수가 1개인 생성자를 호출한다*/

		this.company = company;//this는 내가 현재 속해있는 생성자
	...
	public void tellYourInfo() {
		...
		//아래: 부모클래스에 정의된 멤버메소드 호출. 상속받았으므로 호출 가능함.
		tellYourName();
	}
}
