...
/*
 	오버라이딩:
 	클래스가 상속관계에 있을 때 하위클래스에서 상위클래스의 멤버메소드와
 	완전히 동일한 모양으로 재정의하는 것을 말한다.
 	오버로딩과 다른 점은 매개변수의 갯수, 타입, 반환형이 완전히 똑같은
 	형태로 정의하는 것이다. 하위클래스에서 오버라이딩한 메소드는 
 	상위클래스의 메소드를 가리게 되므로 항상 최하위 클래스에서 오버라이딩한
 	메소드가 호출되게 된다.*/
...
class BaseSpeaker extends Speaker {
	...
	@Override
	public void showState() {
		//super를 통해 부모클레스에 정의된 멤버메소드를 호출한다
		super.showState();
