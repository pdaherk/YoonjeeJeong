...
/* 추상클래스 1: 추상메소드를 포함하지 않는 추상클래스 */
abstract class AbsClass1 {
	//int 형으로 표현할 수 있는 최대값을 반환하는 Wrapper클래스
...
class AbsClassChild1 extends AbsClass1 {
	...
	@Override
	void instanceMethod() {
		System.out.println("필요없다면 오버라이딩 하지 않아도 상관없음");
	}	
}
/*
 추상클래스 2: 추상메소드를 포함한 클래스
	추상메소드를 멤버포 포함한 클래스는 반드시 abstract로 정의해야 함.
...
/*	부모가 가진 추상메소드를 그대로 상속받게 되므로 에러가 나지 않으려면
 	해당클래스도 abstract로 선언하거나, 부모의 abstract 메소드를
 	오버라이딩 처리해야 한다. */
...
public class E01AbstractBasic {
	public static void main(String[] args) {
		...
		//AbsClass1 absClass1 = new AbsClass1(); //error
