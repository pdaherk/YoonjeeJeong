...
/*
 ...
 -반환값이 없다. 즉 특정 연산 후 값을 반환할 수 있다.
 	따라서 void를 쓸 수 없다.
-그 외는 메소드가 가진 모든 특성을 가지고 있다.(매개수전달,
	오버로딩 등)

※생성자 내에서 this의 사용법:
용도 1: 동일 클래스내에서 다른 생성자를 호출할 때 쓴다,
		단 생성자 내에서만 호출 가능하다
		사용법: this(인자1, 인자 2 ...):		
용도 2: 생성자내에서 멤버변후와 매개변수의 이름이
 		동일할 때 구분하기 위해 사용한다.
 		this.변수 => 멤버변수
 		변수 => 생성자의 매개변수
 		이름이 서로 다르다면 this는 생략핻 된다.
...
public class PersonConstructor {
	...
	String addr;
	/*디폴트 생성자:
	 	해당 클래스에 생성자를 정의하지 않은 경우에 아래와 같은
