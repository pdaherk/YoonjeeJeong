...
class Car{	
	...
	//멤버상수
	public static final String AUTO = "수동";
	...
	Human owner;//소유주
		/*	소유주: Human클래스를 기반으로 하는 멤버변수로
		  	초기화를 위해 이름, 나이, 에너지 세가지의 값이 필요하다
		  	클래스를 구성할 때 또다른 객체를 멤버변수로
		  	선언할 수 있다.  */
	
	//멤버메소드
	void drive() {
		System.out.println(owner.name +"가(이) "+ carModel
				+ "을 운전한다");
	}
	/*
	 객체의 초기화를 담당하는 메소드라는 아래는 항상 같은 값으로만 초기화 된다는 한계를 가지고 있다. 
	 */
	void initialize() {
		carModel = "마세라티";
		carYear = 2017;
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		//car1.drive(); 
		/* error발생. (자동차 소유주를 표현하는 Human객체가
			초기화되지 않은 상태에서 출력을 시도하여 에러). */
		
		System.out.println("초기화메소드 호출 후");
		car1.initialize();//고정된 내용으로 초기화 됨
		car1.drive();
		car1.showCarInfo();
		
		
		//자동차2 객체생성: 객체생성 후 멤버변수를 직접 접근하여 초기화한다
		Car car2 = new Car();
		...
		car3.initialize("페라리", 2012, "정윤지", 20, 10);//초기화
