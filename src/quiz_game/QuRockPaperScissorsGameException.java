...
class WrongNumberException extends Exception {
	public WrongNumberException() {
		super("입력이 잘못 되었어요-_- 1~3사이를 입력하세요");
	}
}
...
public class QuRockPaperScissorsGameException {
	...
	public static void rpsGameStart() throws WrongNumberException {
		...
		while(true) {
			...
			//2.사용자 입력
			int user=0;//user를 전역변수로 해서 try문 안에서만 쓰일 수 있는 상황 모면
			try {			 
			...
			if( !(user<1 || user>3) ) {
				...
				System.out.println("가위바위보 할줄 모르나? ^_^+");
			}
					
			//4. 5번의 게임이 되었는지 판단후 재시작/종료 확인
			if(gameCount>=5) {
