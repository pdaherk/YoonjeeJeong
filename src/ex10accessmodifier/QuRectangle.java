...
class Rectangle
{
	//멤버변수
		...
		*/
   		...
   		if(ulx>=lrx || uly>=lry) {
   			System.out.println("좌/우측 좌표범위가 잘못되었습니다");
   			isTrue = false;
   		}
   		//멤버변수 초기화
   		this.ulx = ulx;
   	...
   	public boolean isRange(int point) {
  		if(point<0 || point>100)
   			return false;//0과 100범위를 벗어나면 좌표오류
   		else
   			return true;
   	}
   	public void showArea()
   	{
   		/* 사각형의 넓이를 구하는 메소드 정의 */
   		if(isTrue==false) {
   			System.out.println("좌표값 로유로 넓이계산이 불가합니다");
   			return;
   		}
   		//좌표값을 구하여 넓이 계산
   		int xLength = lrx - ulx;
...
x, y 좌표는 0 ~ 100 사이의 값을 가진다. 즉 좌표값이 마이너스 or 100을 초과하면 
‘좌표 범위가 잘못되었다’라고 print해줘야 한다.우하단의 좌표값이 좌상단의 좌표값보다 작아서는 안됨. 
이 경우 ‘좌표 지정이 잘못되었다’ 라고 print해줘야 한다. 좌표R값이 정상일때만 넓이가 출력된다.
결과] 넓이:81
좌상단 좌표 범위가 잘못되었습니다.
우하단 좌표 범위가 잘못되었습니다.
