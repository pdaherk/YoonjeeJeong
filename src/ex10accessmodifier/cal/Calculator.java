...
/* calculator class는 해당 자바 파일에서 외부에 가장 의미있는 
 클래스로서 public으로 선언 되었다. 
 자바에서는 하나의 클래스만 public으로 선언하는 것이 좋다.
 public으로 선언된 클래스에서 default클래스를 
 사용하는 형태로 제작되어야한다. */
public class Calculator {
	...
	/*
	 생성자메소드: 해당 클래스의 객체가 생성될때 덧셈, 뺄셈을
	...
	/*
	 멤버메소드: 
	 덧셈혹은 뺄셈연산후 결과를 반환하는 메소드로 
	 각 연산을 위해 해당 default클래스의 멤버메소드를 호출한 후 결과를 반환받는다.
	 */
	...
	/*
	 연산의 횟수를 디스플레이 해 주는 메소드( showOperatingTimes(){} )
	 정보는 은닉되어 있는 멤버변수의 출력을 위해 getter() 메소드를 호출하고 있다.
	 */
