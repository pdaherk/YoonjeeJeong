...
public class E02StringMethod {
	...
	public static void main(String[] args) {
		...
		/*2.charAt(인덱스):문자열에서 특정 index에 해당되는
		  			문자 하나를반환한다. index는 0부터 시작한다!! */
		...
		/*4. compareTo(): 두 문자열을 첫번째 문자부터 순차적으로 비교하면서
		 					...
		 					뒤의 문자열의 아스키모드가 크면 음수를 반환
		 					같으면 0을 반환한다*/
		String str3 = "A";//아스키 코드 65
		String str4 = "B";//아스키코드 66
		System.out.println(str3.compareTo(str4));//-1
		System.out.println(str4.compareTo(str3));//1
		System.out.println("ABC".compareTo("ABC")==0 ?//대소문자 구분하니 조심.
				"문자열이 같다 " : "문자열이 다르다");
		
		/*
		 5. concat(): 두개의 문자열을 연결할 때 쓴다. + 기호와 같은 역할을 한다.*/
		System.out.println("JAVA ".concat("WORLD ").concat("Go"));
		System.out.println("JAVA "+"WORLD "+"GO");
		
		/* 6. boolean contains():
		 	문자열에 특정 문자열이 포함되어있는지 판단하여 포함되었으면
		...
		//7] endsWith()
		//특정 문자열이 특정 문자열로 끝나면 true, 아니면 false 반환.
		//8] startsWith()
		//특정 문자열이 특정 문자열로 시작하면 true, 아니면 false 반환.
		System.out.println("www.daum.net".endsWith("net"));
		System.out.println("naver.com".startsWith("http"));
		
		//9]format() 자바내에선 많이 사용 안 된다
		//출력형식을 지정하여 문자열로 반환하고 싶을때 사용
		//printf()와 사용법은 동일하고 주로 웹어플(JSP)를 제작할 때 사용
		...
		//만약 찾는 문자역이 없다면 -1 을 반환한다.-1반환하는 이유: 인덱스는 0부터 시작해서
		System.out.println(str1.indexOf("ava"));//12나옴
		System.out.println(str1.indexOf("J"));//대문자J 없어서 -1반환
		System.out.println(email1.indexOf("@")!=-1 ?
				"이메일형식맞음" : "이멜형식아님");
		/*
		 시나리오] 주민등번을 indexOf를 사용하여 성별을 구분하는 프로그램을 작성하시오		
		 */
		juminNum = "123403-3012345";
		...
		//11]lastIndexOf()
		//사용법은 indexOf와 동일하나 문자열의 뒤에서부터 문자열을찾아서 인덱스를 반환한다.
		//뒤에서부터 카운팅한다고 해서 뒤에것이 0,1,2... 순으로 되는 건 아니다
		System.out.println(str1.lastIndexOf("ava"));
		System.out.println(str1.lastIndexOf("J"));
		System.out.println("indexOf"+ str1.indexOf("a"));//12앞에 있는 a의 인덱스
		System.out.println("lastIndexOf:" +
				str1.lastIndexOf("a"));//14뒤에 있는 a의 인덱스
		//12]replace
		//특정 문자열을 찾아서 지정된 문자열로 변경한다
		//만약 찾는 문자열이 없다면 변경만 되지 않을 뿐 에러발생은 없다
		System.out.println("J를G로 변경하기");
		System.out.println(str1.replace("J", "G"));//무변화
		System.out.println("java를 KOSMO로 변경하기");
		System.out.println(str1.replace("java", "KOSMO"));//KOSMO로 변겨됨
		
		//13]split()
		//문자열을 구분자를 통해 분리해서 String타입의 배열로 반환한다.
		//해당 구분자가 없는 경우에는 크기가 1인 String배열로 반환된다.
		String phoneNum = "010-1234-5678";
		String[] phoneArr = phoneNum.split("-");
		//'-'로 split을 하니 배열이 [010 , 1234 , 5678]이 되고 크기는 3이다.
		for(int i=0 ; i<phoneArr.length ; i++) {
			System.out.printf("phoneArr[%d]=%s\n",
					i, phoneArr[i]);
		}
		phoneArr = phoneNum.split("%");
		//쪼개는 구분자가 없으니 그냥 통으로 나온다 010-1234-5678)
		for(int i=0 ; i<phoneArr.length ; i++) {
			System.out.printf("phoneArr[%d]=%s\n",
					i, phoneArr[i]);}
		...
		/* 시작인덱스와 끝 인덱스 사이의 문자열을 잘라서 반환해줌.
		 ...
		 사용법1: substring(인덱스): 이 경우 인덱스부터 문자열의
		 끝가지를 잘라서 반환한다
		 사용법2: substring(인덱스1, 인덱스2): 인덱스에 포함되는 부분만 
		 잘라서 반환함 */
		System.out.println(str1.substring(3));//3번후를 잘라서 출력
		System.out.println(str1.substring(3, 7));//3과 7사이를 출력
		
		/*시나리오] 다름 파일명에서 확장자를 잘라내는 프로그램을
		 작성하시오. 파일명: my.file.images.jpg */
		String filename = "my.file.images.psd";
