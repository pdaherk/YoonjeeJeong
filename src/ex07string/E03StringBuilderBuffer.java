...
기본 메모리가 소멸되고 새로운 메모리가 생성되는 낭비를 막기 위해서
StringBuffer클래스가 사용된다. 해당 클래스는 새로운 메모리가 
생성안 되고 기존 메모리의 문자열을 변경하게 된다  
 */
public class E03StringBuilderBuffer {

	public static void main(String[] args) {
/*		append():
 		문자열의끝에 새로운 문자열을 연결한다,
 		insert(): 
		...
		System.out.println("저장된 문자열 크기:"+buf.length());
		System.out.println("기본버퍼크기"+ buf.capacity());
		buf.append(" 금일은 StringBuffer 공부중")
			.append("오늘은 수요일");
		System.out.println("저장된 문자열 크기:"+buf.length());
		System.out.println("기본버퍼크기"+ buf.capacity());
