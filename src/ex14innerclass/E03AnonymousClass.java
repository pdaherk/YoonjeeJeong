...
public class E03AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/* Person클래스를 상속받은 익명클래스 정의 및 생성:
		 Person타입의 인스턴스 변수에 Person을 상속받은 익명 클래스를 생성해서
		 그 주소를 할당한다.
		 개념적으로 new Person() {} => 익명extends Person{};
		 형태라 생각하면 된다.
		...
		//anonyPerson.newMethod();//접근불가-에러.
		/*
		 부모타입의 참조변수로 하위객체에서 확장한 멤버에 접근할 
		 수 없므으로 위 부분은 에러가 발생한다.*/
 ...
 */
		...
		//anonyStudent.age = 100;//접근불가-에러
		//((Student)anonyStudent).age = 100;//다운캐스팅 불가=에러
	...
	/*
	 ...
	 따라서 익명클래스는 오버라이딩의 목적으로만 사용해야함.
	 */
	/*
	Person anonyTest = new Student("다움캐스팅", "안되욤") {
		String jumin = "001024";
	};
