...
/*
 ...
 단, 숫자의 갯수는 매개변수로 전달받고 숫자의 갯수만큼 스캐너 클래스를 통해 입력받는다.
...
public class E05MethodType04_2 {
	...
	public static void main(String[] args) {
		...
		System.out.println("최대값1:"+ maxValue1);//4개의 수 중에서 최대값을 출력할 것이다
		
		int maxValue2= returnMaxNumber(6);
		System.out.println("최대값2:"+ maxValue2);//6개의 수 중에서 최대값을 출력할 것이다
	}
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i=1 ; i<=numberCnt ; i++) {
			...
			if(i==1 ) {
				...
				//두번째 입력값부터 maxVal이라고 한다
				if(maxVal<inputNum) {
					/*
					 만약 maxVal보다 입력값이 더 크다면 입력값으로 
					 교체한다.
					 */
					maxVal = inputNum;
				}
