...
public class E06LocalVariable {
	...
	public static void main(String[] args) {
		...
		boolean scope = true;
		/*
		 main 지역에서 num을 선언하면 그보다 작은 if문의
		 지역에서는 같은 이름의 변수를 선언할 수 없다.
		 큰 지역에서 이미 선언하여 메모리상에 상주하고 있기 때문에... 
		 */
		//int num = 9;
		if(scope) {
			int num = 1;
			num++;
			System.out.println("첫번째 if문 지역:"+ num);//왜 결과가 1? num++때문에?
		}
		else {
			int num = 5;
			System.out.println("첫번째 if문의 else지역:"+ num);
			/*
			 if문의 블럭에서 선언된 변수num 은 해당 블럭을 벗어나면
	...
	/*
	 ...
	 simpleFunc() 메소드는 서로 다른 지역이므로 같은 이름의 변수를 선언할 수 있다 
	 */
