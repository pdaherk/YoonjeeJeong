...
public class E05TwoDimArray02 {
	...
	public static void main(String[] args) {
		 ...
		 */
		//Math.random (0~10까지의 실수 출력?
		for(int i=0 ; i<3 ; i++) {//세로에 해당
			for(int j=0 ; j<4 ; j++) {//가로에 해당                
				arr2Dim[i][j] = i + j;
			}
		}
		 //아래에 초기화된 데이터를 출력
		...
		for(int i=0 ; i<twoArray.length ; i++) {
			//안쪽은 가로크기만큼 반복(배열명[인덱스]를 이용해 각 행의 크기를 가져옴)
		...
		//호출해주는 것이 좋다.
		showArray(twoArray);//배열 출력
		/* 배열에 각 요소값을 전달한 매개변수만큼 증가시킴. 여기서는 10만큼 증가.
		 */
