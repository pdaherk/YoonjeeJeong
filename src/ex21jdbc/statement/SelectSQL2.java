...
/* ResultSet 클래스
	...
	오라클의 자료형이
		number타입 : getInt()
		car/varchar2타입 : getString()
		date타입 : getDate()
		메소드로 각 컬럼의 데이터를 추출한다.
	-인자는 select 절의 컬럼 순서대로 인덱스(1부터 시작)을 사용하거나
	컬럼명을 사용할 수 있다.
		...
		try {
			...
			while(rs.next()) {
				...
				String regidate = 
						...
						날짜와 시간까지만 잘라서 출력(분 이후로 출력 안 됨)*/
				
				/*
				 오라클의 date형을 getString()으로 추출하면
				 2020-03-25 11:40:46 형태로 시간까지 출력할 수 있다.
				 만약 문자열을 잘라서 출력하고 싶을 때는 substring()을
				 활용한다.
				 */
